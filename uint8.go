...
func MakeUint8(name string, buf ...int) (chan<- uint8, <-chan uint8) {
	...
	go wIfClient(U.selw, Tuint8, U.name, U.idx, 1)
	go rIfClient(U.selr, Tuint8, U.name, U.idx, 1)
	go wIfClient(U.w, Tuint8, U.name, U.idx, 0)
	go rIfClient(U.r, Tuint8, U.name, U.idx, 0)
	go U.selsend()
	go U.selrecv()
